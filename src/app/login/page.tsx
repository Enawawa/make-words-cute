            ...
            {(["email", "google"] as Tab[]).map((t) => (
              <button
                ...
                className={`flex-1 py-3.5 text-sm font-semibold transition-colors ${
                  tab === t
                    ? "text-[var(--color-primary)] border-b-2 border-[var(--color-primary)] bg-pink-50/50"
                    : "text-gray-400 hover:text-gray-600"
                }`}
              >
                {t === "email" ? "📧 邮箱验证码" : "🔵 Google 登录"}
              </button>
            ))}
          </div>
            ...
            {tab === "google" && (
              <div className="space-y-4">
                <p className="text-sm text-center text-[var(--color-text-muted)]">
                  使用 Google 账号一键登录，安全便捷
                </p>
                <button
                  onClick={handleGoogleLogin}
                  className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl border-2 border-gray-200 bg-white hover:bg-gray-50 font-semibold text-gray-700 transition-all hover:scale-[1.01] active:scale-[0.99]"
                >
